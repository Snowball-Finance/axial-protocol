...
async function main() {
  ...
  console.log("emergency withdrawing to the following account:", signer.address);
  const teddyRewarder_addr = "0xd84f5C83b2Dc67D9aC7Af06A93509CCbe4E81aEa";

  const Rewarder = new ethers.Contract(teddyRewarder_addr, SimpleRewarderPerSec_ABI, signer);
  const withdraw = await Rewarder.emergencyWithdraw();
  const tx_withdraw = await withdraw.wait(1);
  if (!tx_withdraw.status) {
    console.error(`Error running emergency withdraw`);
    return;
  }
  const newRate = await Rewarder.tokenPerSec();
  console.log(`successfully ran emergency withdraw`);
