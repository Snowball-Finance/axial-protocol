...
async function main() {
  ...
  const setRate = await Rewarder.setRewardRate("0x547DEA7552034D8"); //"380517503805175000"
  const tx_setRate = await setRate.wait(1);
  if (!tx_setRate.status) {
    console.error(`Error setting the rate for the TEDDY Rewarder`);
    return;
  }
  const newRate = await Rewarder.tokenPerSec();
  console.log(`set the rate for the TEDDY Rewarder to ${newRate}`);
