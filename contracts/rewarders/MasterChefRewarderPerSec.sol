...
/**
 * This is a sample contract to be used in the MasterChefAxialV2 contract for partners to reward
...
contract MasterChefRewarderPerSec is IRewarder, Ownable {
    ...
    IMasterChefAxialV2 public immutable MCV2;
    ...
    /// @dev Info of each MCV2 poolInfo.
    /// `accTokenPerShare` Amount of AXIAL each LP token is worth.
    /// `lastRewardTimestamp` The last time AXIAL was rewarded to the poolInfo.
    struct PoolInfo {
    ...
    constructor(
        ...
        require(Address.isContract(address(_MCV2)), "constructor: MasterChefAxialV2 must be a valid contract");
    ...
    /// @dev Deposits a dummy token to a MaterChefV1 farm so that this contract can claim reward tokens.
    ...
    /// @dev Sets the allocation point. THis will also update the poolInfo.
    ...
    /// @dev Claims reward tokens from MCV1 farm.
    function harvestFromMasterChefV1() public {
        MCV1.deposit(MCV1_pid, 0);
    }

    /// @dev Function called by MasterChefAxialV2 whenever staker claims AXIAL harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onAxialReward(address _user, uint256 _lpAmount) external override onlyMCV2 {
        updatePool();
