...
contract MasterChefAxial is Ownable {
    ...
    function pendingAxial(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 axialReward = multiplier
                .mul(axialPerSec)
                .mul(pool.allocPoint)
                .div(totalAllocPoint)
        ...
        return user.amount.mul(accAxialPerShare).div(1e12).sub(user.rewardDebt);
    }
