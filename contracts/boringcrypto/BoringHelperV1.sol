...
interface IMasterChef {
    ...
    function axialPerBlock() external view returns (uint256);
...
contract BoringHelperV1 is Ownable {
    ...
    address public maker; // IAxialMaker(0xE11fc0B43ab98Eb91e9836129d1ee7c3Bc95df50);
    IERC20 public axial; // IAxialToken(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    IERC20 public WAVAX; // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    IFactory public axialFactory; // IFactory(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac);
    ...
    function getAVAXRate(IERC20 token) public view returns (uint256) {
        ...
        if (axialFactory != IFactory(0)) {
            pairAxial = IPair(axialFactory.getPair(token, WAVAX));
        }
        if (address(pairPangolin) == address(0) && address(pairAxial) == address(0)) {
            return 0;
        }
        ...
        if (address(pairAxial) != address(0)) {
            (uint112 reserve0Axial, uint112 reserve1Axial, ) = pairAxial.getReserves();
            reserve0 += reserve0Axial;
            reserve1 += reserve1Axial;
            if (token0 == IERC20(0)) {
                token0 = pairAxial.token0();
            }
        }
    ...
    struct UIInfo {
        uint256 avaxBalance;
        uint256 axialBalance;
        uint256 axialBarBalance;
        uint256 xaxialBalance;
        uint256 xaxialSupply;
        uint256 axialBarAllowance;
        Factory[] factories;
        uint256 avaxRate;
        uint256 axialRate;
        uint256 btcRate;
    ...
    function getUIInfo(
        ...
        if (axial != IERC20(0)) {
            info.axialRate = getAVAXRate(axial);
            info.axialBalance = axial.balanceOf(who);
            info.axialBarBalance = axial.balanceOf(address(bar));
            info.axialBarAllowance = axial.allowance(who, address(bar));
        }

        if (bar != IERC20(0)) {
            info.xaxialBalance = bar.balanceOf(who);
            info.xaxialSupply = bar.totalSupply();
        }
        ...
        if (chef != IMasterChef(0)) {
            ...
            for (uint256 i = 0; i < poolLength; i++) {
                (uint256 pendingAxialAmt, , , ) = chef.pendingTokens(i, who);
                pendingAxial += pendingAxialAmt;
            }
            info.pendingAxial = pendingAxial;
        }
    ...
    function pollPools(address who, uint256[] calldata pids) public view returns (UserPoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            ...
            (uint256 pendingAxial, , , ) = chef.pendingTokens(pids[i], who);
            pools[i].pending = pendingAxial;
