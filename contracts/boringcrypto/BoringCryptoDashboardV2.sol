...
contract BoringCryptoDashboardV2 {
    ...
    function getAVAXRate(address token) public view returns (uint256) {
        ...
        if (token != wavax) {
            ...
            pairAxial = IPair(IFactory(axialFactory).getPair(token, wavax));
            ...
            uint112 reserve0Axial;
            uint112 reserve1Axial;
            ...
            if (address(pairAxial) != address(0)) {
                (reserve0Axial, reserve1Axial, ) = pairAxial.getReserves();
            }

            if (address(pairAxial) == address(0) || reserve0Pangolin > reserve0Axial || reserve1Pangolin > reserve1Axial) {
                ...
                if (pairAxial.token0() == wavax) {
                    avax_rate = uint256(reserve1Axial).mul(1e18).div(reserve0Axial);
                } else {
                    avax_rate = uint256(reserve0Axial).mul(1e18).div(reserve1Axial);
                }
            }
        }
        return avax_rate;
