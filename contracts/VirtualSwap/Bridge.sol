...
/**
 * @title Bridge
 * @dev This contract is responsible for cross-asset swaps using the Synthetix protocol as the bridging exchange.
...
contract Bridge is ERC721 {
    ...
    /**
     * @dev Deploys this contract and initializes the master version of the SynthSwapper contract. The address to
    ...
    /**
     * @dev Returns the address of the proxy contract targeting the synthetic asset with the given `synthKey`.
    ...
    /**
     * @dev Returns various information of a pending swap represented by the given `itemId`. Information includes
    ...
    /**
     * @dev Settles and withdraws the synthetic asset without swapping it to a token in a Saddle pool. Only the owner
    ...
    /**
     * @dev Completes the pending `tokenToSynth` swap by settling and withdrawing the synthetic asset.
    ...
    /**
     * @dev Calculates the expected amount of the token to receive on calling `completeToToken()` with
    ...
    /**
     * @dev Completes the pending `SynthToToken` or `TokenToToken` swap by settling the bridging synth and swapping
    ...
    /**
     * @dev Calculates the expected amount of the desired synthetic asset the caller will receive after completing
    ...
    /**
     * @dev Initiates a cross-asset swap from a token supported in the `swap` pool to any synthetic asset.
    ...
    /**
     * @dev Calculates the expected amount of the desired token the caller will receive after completing
    ...
    /**
     * @dev Initiates a cross-asset swap from a synthetic asset to a supported token. The caller will receive
    ...
    /**
     * @dev Initiates a cross-asset swap from a token in one Saddle pool to one in another. The caller will receive
     * an ERC721 token representing their ownership of the pending cross-asset swap.
     * @param swaps the addresses of the two Saddle pools used to do the cross-asset swap
     * @param tokenFromIndex the index of the token in the first `swaps` pool to swap from
     * @param tokenToIndex the index of the token in the second `swaps` pool to swap to
    ...
    /**
     * @dev Registers the index and the address of the supported synth from the given `swap` pool. The matching currency key must
    ...
    /**
     * @dev Returns the index of the supported synth in the given `swap` pool. Reverts if the `swap` pool
     * is not registered.
     * @param swap the address of the pool that contains the synth
    ...
    /**
     * @dev Returns the address of the supported synth in the given `swap` pool. Reverts if the `swap` pool
     * is not registered.
     * @param swap the address of the pool that contains the synth
    ...
    /**
     * @dev Returns the currency key of the supported synth in the given `swap` pool. Reverts if the `swap` pool
     * is not registered.
     * @param swap the address of the pool that contains the synth
    ...
    /**
     * @dev Updates the stored address of the `EXCHANGER` contract. When the Synthetix team upgrades their protocol,
