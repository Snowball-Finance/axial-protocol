...
describe("StakingVe", () => {
  ...
  it("claiming repeatedly does not affect a different users funds, or allow user to claim more than they are owed", async () => {
    ...
    let inWallet = await axialTokenMock.balanceOf(await alice.getAddress());
    await stakingVe.connect(alice).ClaimMyFunds();
    let inWalletAfterClaimingTwice = await axialTokenMock.balanceOf(await alice.getAddress());
    await stakingVe.connect(bob).ClaimMyFunds();
    let inBobsWallet = await axialTokenMock.balanceOf(await bob.getAddress());
  ...
  it("User can create a gigafuckton of locks", async () => {
    ...
    await axialTokenMock.connect(deployer).mints([await alice.getAddress()], [99999999]);
    await axialTokenMock.connect(alice).approve(stakingVe.address, '99999999');

    for (let i = 0; i < 1000; ++i) {
      await stakingVe.connect(alice).CreateLock(SECONDS_IN_A_YEAR * 2, (10 + i).toString());
      await increaseTimestamp(SECONDS_IN_A_WEEK);
      await stakingVe.connect(alice).ClaimMyFunds();
  ...
  it.only("User can create a new lock every day with their rewards", async () => {
    ...
    for (let i = 0; i < 1000; ++i) {
      ...
      let power = await stakingVe.connect(alice).GetMyPower();
      inWallet = await axialTokenMock.balanceOf(await alice.getAddress());
      console.log("Day %d, Balance: %d Power: %d, Wallet: %d", i, balance, power, inWallet);
    }
  });
