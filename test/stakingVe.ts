...
describe("StakingVe", () => {
  ...
  */
  ...
  it("user can not create lock with 0 duration", async () => {
    await axialTokenMock.connect(alice).approve(stakingVe.address, "10")
    expect(stakingVe.connect(alice).CreateLock("0", "10")).to.be.revertedWith(
  ...
  it("user can create multiple locks", async () => {
    ...
    for (let i = 0; i < locks.length; i++) {
      ...
      expect(locks[i].EndBlockTime.sub(locks[i].StartBlockTime)).to.almost(
        duration,
      )
    }
  })

  it.only("multiple users can create a lock", async () => {
    await axialTokenMock.connect(alice).approve(stakingVe.address, "10")
  ...
  it("user should be able to claim their tokens", async () => {
    ...
    expect(await axialTokenMock.balanceOf(await alice.getAddress())).to.eq(
      balance,
    )

    await increaseTimestamp(TWO_YEARS / 2)
    await stakingVe.connect(alice).ClaimMyFunds()
    expect(await axialTokenMock.balanceOf(await alice.getAddress())).to.eq(10)
  })
  ...
  it("user should not be able to steal funds", async () => {
    ...
    expect(await axialTokenMock.balanceOf(await alice.getAddress())).to.be.gt(0)
