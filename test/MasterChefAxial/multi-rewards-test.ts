...
import {MultiRewarderContract} from "../mocks/MultiRewarder";
import { addRewardToken, balanceOfRewardTokens } from "../mocks/InternalFunctions";
...
const doMultiRewardsTest = () => {
   ...
   describe("Tests for multi-rewards:", async () => {
      ...
      before(async () => {
         ...
         lp = await generateLPToken("LPToken", "LP");
        
         /** MultiRewarder Mock **/ 
         MultiRewarder = await MultiRewarderContract( 
            lp,  
            [asset1, asset2, asset3, asset4], 
            ["250000000000", "25000000", "4999993393", "399920440"],
      ...
      it("should check add a reward token to our list of reward tokens", async function () {
         await addRewardToken(MultiRewarder, governanceSigner, 239042309532, wallet_addr, walletSigner); 
      })
