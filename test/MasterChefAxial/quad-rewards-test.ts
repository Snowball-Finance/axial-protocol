...
import { ADDRESS_ZERO, advanceTimeAndBlock, advanceBlockTo, latest, duration, increase } from "../../utils"

describe("MasterChefAxialV3", function () {
  before(async function () {
    ...
    this.MCV2 = await ethers.getContractFactory("MasterChefAxialV3")
  ...
  context("With ERC/LP token added to the field and using SimpleRewarderPerBlock", function () {
    ...
    it("should check rewarder's arguments are contracts", async function () {
      ...
      ).to.be.revertedWith("constructor: MasterChefAxialV3 must be a valid contract")
    })
    ...
    it("should allow emergency withdraw from MasterChefAxialV3", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.joe.address,
        this.dev.address,
    ...
    it("should only allow MasterChefAxialV3 to call onJoeReward", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.joe.address,
        this.dev.address,
