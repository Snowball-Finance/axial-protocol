...
describe('Axial Router - swap', () => {
    ...
    it('Router swap matched the query - multiple hops', async () => {
        ...
        WAVAX = await ethers.getContractAt('WAVAX', assets.WAVAX)
        await WAVAX.connect(trader).deposit({ value: amountIn })
        // Approve for input token
        await helpers.approveERC20(trader, result.path[0], AxialRouter.address, ethers.constants.MaxUint256)
        // Do the swap
        await AxialRouter.connect(trader).swapNoSplit(
            [
                result.amounts[0], 
        ...
        outputTokenContract = await ethers.getContractAt('ERC20', tokenOut)
    ...
    it('Router swap matched the query - with permit', async () => {
        ...
        const chainId = 43114
        // Permit setting
        let inputTokenContract = await ethers.getContractAt('contracts/interfaces/IERC20.sol:IERC20', tokenIn)
        ...
        await AxialRouter.connect(bob).swapNoSplitWithPermit(
            [
                result.amounts[0], 
                result.amounts[result.amounts.length-1],
                result.path,
        ...
        outputTokenContract = await ethers.getContractAt('contracts/interfaces/IERC20.sol:IERC20', tokenOut)
    ...
    it('Mix swap can be done to AVAX', async () => {
        ...
        // Top up trader with starting tokens
        WAVAX = await ethers.getContractAt('contracts/interfaces/IWavax.sol:IWAVAX', assets.WAVAX)
        await WAVAX.connect(trader).deposit({ value: amountIn })
        await helpers.topUpAccountWithToken(
            trader, 
            trader.address,
        ...
        // Approve for input token
        await helpers.approveERC20(trader, tokenIn, AxialRouter.address, ethers.constants.MaxUint256)
        let balBefore = await ethers.provider.getBalance(trader.address)
        // Do the swap
        const swap = AxialRouter.connect(trader).swapNoSplitToAVAX(
            [
                result.amounts[0], 
                result.amounts[result.amounts.length-1],
                result.path,
    ...
    it('Mix swap can be done from AVAX', async () => {
        ...
        // Do the swap
        await AxialRouter.connect(trader).swapNoSplitFromAVAX(
            [
                result.amounts[0], 
                result.amounts[result.amounts.length-1],
                result.path,
    ...
    it('Transactions reverts if expected out-amount is not within slippage', async () => {
        ...
        // Do the swap
        await expect(AxialRouter.connect(trader).swapNoSplitFromAVAX(
            [
                result.amounts[0], 
                result.amounts[result.amounts.length-1],
                result.path,
    ...
    it('Swap with 3-4 steps', async () => {
        ...
        await helpers.approveERC20(trader, tokenIn, AxialRouter.address, ethers.constants.MaxUint256)
        // Do the swap
        const swap = () => AxialRouter.connect(trader).swapNoSplit(
            [
                result.amounts[0], 
                result.amounts[result.amounts.length-1],
                result.path,
        ...
        const tokenOutContract = await ethers.getContractAt('contracts/interfaces/IERC20.sol:IERC20', tokenOut)
